...
/**
 * Tells the user, at the start of a session, that a newer ProcerERP plugin has been published.
 *
 ...
 * - `additionalContext` is given to the model, so "update procer-distribution" is understood without a search.
 *
...
const publishedManifestUrl = (repository) => {
  ...
  return `https://raw.githubusercontent.com/${path}/main/plugins/procer-distribution/.claude-plugin/plugin.json`
}
...
const report = () => {
  ...
  const message = `ProcerERP ${latest} is available - you have ${installed}. Ask me to "update procer-distribution" and I will do it.`
  ...
  process.stdout.write(
    `${JSON.stringify({
      ...
      hookSpecificOutput: {
        ...
        additionalContext:
          `The installed ProcerERP plugin is ${installed} and ${latest} is published. If the user asks to ` +
          "update ProcerERP, or hits a ProcerERP tool that does not match what a skill describes, run the " +
          "procer-distribution-update skill. Do not offer this unprompted more than once in a session.",
      },
    })}\n`,
  )
}
