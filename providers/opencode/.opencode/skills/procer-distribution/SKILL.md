...
The server is declared under `mcp` in `opencode.json` at the repository root. Reload OpenCode after
the file changes; it does not pick up new servers mid-session.
