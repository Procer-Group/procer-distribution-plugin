...
const USAGE = `procer-distribution install — installs the ProcerERP skill and MCP server into this project.

  procer-distribution install                       install for every harness detected on this machine
  procer-distribution install --harness cursor      install for one harness
  procer-distribution install --from <dir>          install from a local build instead of the published trees
...
const main = () => {
  ...
  if (providerIds.length === 0) {
    ...
    console.log(`Name one explicitly: procer-distribution install ${HARNESS_ARGUMENT} <${DETECTABLE_PROVIDER_IDS.join("|")}>`)
    return
  }
  ...
  console.log("")
  console.log("Verify: start the harness and ask it to call the procer-distribution server_info tool.")
...
const report = (result) => {
  ...
  if (result.pinning === DYNAMIC_REGISTRATION) {
    console.log("  NOTE    This harness signs in by dynamic client registration, which the ProcerERP")
    console.log("          realm refuses. If sign-in fails, its config must name the OAuth client")
    console.log("          procer-public-client. There is no way around this from the client side.")
  }
}
...
try {
  ...
  console.error(`procer-distribution: ${error.message}`)
  process.exitCode = 1
}
